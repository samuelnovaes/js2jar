...
if(file){
	...
	fs.writeFile(`${className}.java`, javaCode, "utf-8", err=>{
		...
		compileClass.stderr.on('data', data=>console.log(clc.redBright(`stderr: ${data}`)));
		compileClass.on('close', (code) => {
			if(code == 0){
				...
				compileJar.stderr.on('data', data=>console.log(clc.redBright(`stderr: ${data}`)));
				compileJar.on('close', (code) => {
					if(code == 0){
						...
						runJar.stderr.on('data', data=>console.log(clc.redBright(`stderr: ${data}`)));
					}
				});
			}
		});
